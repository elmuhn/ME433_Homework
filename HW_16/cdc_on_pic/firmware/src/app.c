    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_CHECK_CDC_READ:
            ...
            if(appData.isReadComplete == true)
            {
                ...
                // error corresponds to the difference between center of screen (center) and center of mess from android (num1)
                int error, center; 
                center = 320;
                error = center-num1;
                
                // assign phase bits via RB7 and RB14
                LATBbits.LATB7 = 0;
                LATBbits.LATB14 = 1;
                
                // if the center of mess doesn't match with the screen then make turn
                if (200>error>100){
                    OC1RS = 0*(PR2+1)/100;
                    OC2RS = 30*(PR2+1)/100;
                }
                
                // if the center of mess is away from the screen then make a harder turn
                else if (error>200){
                    OC1RS = 60*(PR2+1)/100;
                    OC2RS = 0*(PR2+1)/100;
                }
                ...
                //If the line is centered go straight 
                else if (error==0){
                    OC1RS = 70*(PR2+1)/100;
                    OC2RS = 70*(PR2+1)/100;
                }
                
                // else corresponds when the line doesn't match with the screen in small ranges, then go straight as well
                else {
                    OC1RS = 70*(PR2+1)/100;
                    OC2RS = 70*(PR2+1)/100;
                }

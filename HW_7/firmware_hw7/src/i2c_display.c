...
void display_now(char *message, int now_row, int now_col){
	int i = 0, j, k;
    while(message[i]){
    	int char_row = ascii_table(message[i]);
        if(now_col > 125){
        	now_col = 0;
            now_row = now_row + 8;
        }
        ...
        for(j = 0; j < 5; j++){
        	for(k = 0; k < 8; k++){
            	int char_hex = ASCII[char_row][j];
                int char_bin = (char_hex & (1 << k)) >> k;
                row = now_row + k;
                col = now_col + j;
                display_pixel_set(row, col, char_bin);
            };
        };
        i++;
        now_col = now_col + 5;
...
void display_bar(int coord_x, int coord_y){
    ...
    int row_center = 64/2;
    int col_center = 128/2;

    int x_line, y_line;
    x_line = coord_x*RATIO;
    y_line = coord_y*RATIO;
    ...
    int w;
    while(x_line > 0){
        row_center = row_center + 1*dir_x;          
        for(w = 0; w < 5; w++){          
           display_pixel_set(row_center, col_center+w, 1); 
        };
        x_line = x_line - 1;
    }
    ...
    int h;
    while(y_line > 0){
        col_center = col_center + 1*dir_y;
        for(h = 0; h < 5; h++){
           display_pixel_set(row_center + h, col_center, 1); 
        };
        y_line = y_line - 1;
    }

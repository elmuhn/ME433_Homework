...
int main ( void )
{
    ...
    acc_setup();            //Initialize the accelerometer
    display_init();         //Initialize the display
    display_clear();        //Clear the display

    char message[20];
    short accel[3];   //accelerations for the 3 axes
    short mag[3];     //magnetometer for the 3 axes
    short temp;       //temperature
    
    display_now(message,15,30);
    display_draw();
